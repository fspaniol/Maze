...
void printPossibleMoves(Maze maze) {
  Direction dir[6]={UP, DOWN, NORTH, EAST, SOUTH, WEST};
    ...
    printPositionWithNewline(maze.pos);
    printf("Your possible moves are: \n");
    for (int i = 0; i<6;i++)
    {
        int possibility = canMoveTo(maze,dir[i]);
        char dirstr[6];
        strcpy(dirstr,convertDirectionToString(dir[i]));
         if(possibility == 1)
         {
             printf("\t%s: ALLOWED\n", dirstr);
         }
...
int isSolved(Maze maze) {
    ...
    int flag;
  if(maze.pos.x == maze.goal.x && maze.pos.y == maze.goal.y && maze.pos.z == maze.goal.z)
      flag = 1;
  else flag = 0;
...
int makeMove(Maze *maze, Direction dir) {
    ...
    int flag = canMoveTo(*maze, dir);
    char dirstr[6];
    strcpy(dirstr,convertDirectionToString(dir));
    if (flag == 1)
    {
        switch (dir) {
